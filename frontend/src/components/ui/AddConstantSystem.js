...
const AddConstantSystem = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (response.data) {
          const { id, created_at, updated_at, effectif_fix, ...filteredData } = response.data;

          if (effectif_fix && effectif_fix.length > 0) {
            const { id, created_at, updated_at, system_constant_id, ...effectifFixData } = effectif_fix[0];
            setFormData({
              ...filteredData,
              ...effectifFixData
            });
  ...
  const handleSubmit = async (e) => {
    ...
    const { 
      ...
      Coupe, Production, Repassage_final, Contrôle_final, Magasin_final,
      Magasin_fournitures, Achats_Logistique, Transit, Comptabilité_Finances,
      RH, Ménage, Autres, ...dataToSubmit 
    } = formData;

      ...
      <div className='w-2/3 md:w-5/6 bg-blue-500 h-full flex justify-between items-center'>
        ...
        <div className='items-center flex relative'>
          <div className='mr-5 cursor-pointer' onClick={toggleShowNavMenu}>
            <img alt='user account' className='rounded-full object-cover w-[45px] h-[45px]' src={profileImage} />
          </div>
          {profileImageClicked && (
            <div className='absolute top-10 right-5 mt-2 w-48 bg-white border rounded shadow-md'>
              ...
              <button onClick={handleLogout} className='block w-full text-left px-4 py-2 text-red-500 hover:bg-gray-200'>Logout</button>
            </div>
          )}
        </div>
      </div>
